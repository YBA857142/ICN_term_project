...
# Configuration
HOST = "localhost"
PORT = 8002  # Changed port
SERVER_NAME = "Backend Server 2" #Added server name
...
def handle_client(client_socket, client_address, port):
    try:
        ...
        if request.startswith("GET /index.html"):
            response = "HTTP/1.1 200 OK\nContent-Type: text/html\n\n<html><body><h1>Hello from Backend Server 2!</h1><p>This is index.html</p></body></html>"
            log(port, client_address, 200, "-", "/index.html")
        elif request.startswith("GET /helloworld.html"):
            response = "HTTP/1.1 200 OK\nContent-Type: text/html\n\n<html><body><h1>Hello from Backend Server 2!</h1><p>This is helloworld.html</p></body></html>"
            log(port, client_address, 200, "-", "/helloworld.html")
        elif request.startswith("GET /favicon.ico"):
            response = "HTTP/1.1 404 Not Found\nContent-Type: text/html\n\n<html><body><h1>404 Not Found from Backend Server 2</h1></body></html>"
